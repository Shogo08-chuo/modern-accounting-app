...
type JournalEntryPayload = {
  entryDate: string
  description: string
  lines: JournalEntryPayloadLine[]
}

function parseJournalEntriesQuery(query: Record<string, string | undefined>) {
  ...
  if (accountIdParam && Number.isNaN(accountId)) {
    return { error: 'accountId は数値で指定してください' as const }
  }

  const dateFrom = dateFromParam ? new Date(dateFromParam) : undefined
  if (dateFromParam && Number.isNaN(dateFrom?.getTime())) {
    return { error: 'dateFrom は有効な日付で指定してください' as const }
  }

  const dateTo = dateToParam ? new Date(dateToParam) : undefined
  if (dateToParam && Number.isNaN(dateTo?.getTime())) {
    return { error: 'dateTo は有効な日付で指定してください' as const }
  }
...
function validateJournalEntryPayload(body: unknown):
  ...
  const normalizedLines: JournalEntryPayloadLine[] = []
